...
void MainLight_float(float3 WorldPos, int1 lightOrder, out float3 Direction, out float3 Color, out float DistanceAtten, out float ShadowAtten)
{
...
#endif
	Light mainLight = GetAdditionalLight(lightOrder, WorldPos); // GetMainLight(shadowCoord);
...
void MainLight_half(float3 WorldPos, int1 lightOrder, out half3 Direction, out half3 Color, out half DistanceAtten, out half ShadowAtten)
{
#if SHADERGRAPH_PREVIEW
